...
- 验收时间：2026-08-18 11:18–11:48（Asia/Shanghai）
- 验收分支：`docs/release-candidate`
- 功能提交：`d3547c9`、`9e9164a`、`282b858`
- 验收人：Codex
- 当前结论：Codex 自动验收通过，等待用户最终确认
...
4. 新增 API 示例、并发证据、部署、故障排查、五分钟演示和最终 PR 描述；
...
最初两套控制面会把对方 sandbox 视为孤儿，这是本阶段真实验收发现的问题。修复后，8000 使用默认 namespace，18000 使用 `ccu-release-clean`，同时执行 `smoke.ps1`：
...
验收后仅删除 `ccu-release-clean` 的容器、网络、`postgres-data`/`redis-data` 两个临时卷和临时 API 镜像。该临时数据不可恢复，但只含自动冒烟记录；当前 8000 环境和正式项目卷保留且 readiness=200。
...
- 从空构建层验证：不是依赖旧 Docker 镜像假装 README 可复现；
...
- Git 历史安全扫描：不仅扫描当前工作树，也扫描已经删除的历史内容和历史大对象；
...
1. 按 README 在新目录独立复现；
2. 根据 `docs/demo-script.md` 录制 4:30–5:00、有真人声音的视频；
3. 邀请题目指定的四位 GitHub 协作者；
4. 用无痕窗口或其他账号验证仓库与视频权限；
5. 创建阶段 PR，并提交仓库和视频链接。

## 当前边界

- 发布候选执行路径使用确定性 Fake Agent；
- Anthropic callback adapter 已测试，但真实模型到每会话远程桌面的工具桥未接入；
- 本地 Docker socket、随机 noVNC 高端口和无登录 API 不是不可信多租户生产边界；
- 无缓存构建产生约 2.97 GB 可回收 BuildKit cache；未自动全局 prune，避免影响用户的其他项目构建缓存。
